...
for ta = 1:length(tanks)
    ...
    for pos = 1:length(Positions)
        ...
        for bl = 1:length(Blocks)
            ...
            if exist([block_path '\bst_' Sorter '_mua.mat'])
                load([block_path '\bst_' Sorter '_mua.mat']) % load the bst
                ...
                for un = 1:length(Units)
                    ...
                    DEVaP = vertcat(oddData.('ODD-ASC-DEV-P'){:});
                    DEVbP = vertcat(oddData.('ODD-DES-DEV-P'){:});
                    spikesPdev = [DEVa;DEVbP];
                    ...
                    print(gcf,'-vector','-depsc',[savpath '\' num2str(Units(un)) '_PSTH' Sorter '_MUA.eps'])
                    print(gcf,'-dpng','-r150',[savpath '\' num2str(Units(un)) '_PSTH' Sorter '_MUA.png'])
                    close all
...
for i = 1:height(spiketable)
    ...
    load([Main_path Block(1:slashlocs(2)) '\ClusterGoodLocsMUA.mat'])
    unitloc = find(ClusterMUA(1,:) == spiketable.Unit(i));
    spiketable.unitloc(i) = double(ClusterMUA(2,unitloc));
end

Main_path = 'D:\Jazmin\MultichannelDataTanks\HIP\Marzo2025\';
save([Main_path 'spiketable' Sorter '_MUA.mat'],'spiketable') % save the spiketable
